...
Use these alongside `learn/10_odin_for_js_devs/`.
...
Use these alongside `learn/90_production_with_sauce/06_what_is_sokol.md` and
...
Use these alongside `learn/80_design/coop_lovers_puzzle/`.
...
- **"Juice it or lose it" (Martin Jonasson & Petri Purho)** — why game feel
  matters; maps directly to your `t10_particles_screenshake` and `40_vfx/` lessons.
...
## 6. Networking / Online Co-op (read LAST)

For your online "create a room, friend connects" goal. Full guidance with verified
links lives in its own folder — do not start until your LOCAL game is fun.

- `learn/85_networking/` — reality check, web-dev concept mapping, the
  network-ready input pattern, the Steam path, the raw-sockets path, and
  `learn/85_networking/RESOURCES.md` with all external networking links.
