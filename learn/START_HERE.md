...
cd learn/solutions/odin_for_js_devs/o01_first_program
zsh build.sh
```
...
cd learn/solutions/fundamentals/t01_hello_window
zsh build.sh
```
...
Full version of this method: `learn/LEARNING_METHOD.md`.
...
```
odin_for_js_devs/      o01..o16   learn the language (JS comparison)
game_thinking_for_web_devs/ g01..g08  learn the mindset (no code)
fundamentals/          t01..t11   window, input, gravity, tilemap, camera, shaders
fundamentals/t12       integration room: wire t01..t11 into one playable game
vfx/                   v01..v04   glow, elements, fire, lasers
projects/              sokoban, card game   first full games (standalone)
co_op/                 asymmetric two-player puzzle prototype
production_with_sauce/  how to rebuild it all inside the real sauce/ engine
...
- **Compiler error you don't understand:** read
  `learn/odin_for_js_devs/o15_reading_compiler_errors/LESSON.md`.
- **Want me (or anyone) to review your code:** fill in
  `learn/ATTEMPT_TEMPLATE.md` and `learn/REVIEW_TEMPLATE.md`.
- **Ready to move a standalone game into `sauce/`:** read
  `learn/SAUCE_MIGRATION_TICKETS.md` — but only after the standalone version
  works and you can explain it.
- **Want another explanation, a video, or real shipped-game code:** see
  `learn/RESOURCES.md` (curated external links, organized by phase).
- **Want to work on YOUR co-op "two lovers" game idea:** see
  `learn/design/coop_lovers_puzzle/` — design doc + how to build it with or
...
|------|-----------------|
| `START_HERE.md` | Right now. The front door. |
| `MASTER_TICKET_LIST.md` | Daily. Your ordered checklist. |
| `LEARNING_METHOD.md` | Once, to internalize the daily loop. |
| `README.md` | Reference. Map of folders. |
| `RESOURCES.md` | When you want a video/doc/example for another angle. |
| `design/coop_lovers_puzzle/` | When working on YOUR two-lovers game idea. |
| `ATTEMPT_TEMPLATE.md` | When you submit code for review. |
| `REVIEW_TEMPLATE.md` | When you review code. |
| `SAUCE_MIGRATION_TICKETS.md` | Only when moving a game into `sauce/`. |
