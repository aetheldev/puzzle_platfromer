...
sound_init :: proc() {
	...
	fmod_error_check(System_LoadBankFile(sound_state.system, "res/fmod/Master.bank", LOAD_BANK_NORMAL, &sound_state.bank))
	fmod_error_check(System_LoadBankFile(sound_state.system, "res/fmod/Master.strings.bank", LOAD_BANK_NORMAL, &sound_state.strings_bank))
...
sound_update :: proc(listener_pos: Vec2, master_volume: f32) {
	using fstudio
	assert(sound_state.initialized, "sound system not initted yet")
	...
	fmod_error_check(System_Update(sound_state.system))
	...
	attributes.up = {0, 1, 0};
	fmod_error_check(System_SetListenerAttributes(sound_state.system, 0, attributes, nil));
}

sound_play :: proc(name: string, pos := INVALID_POS, cooldown_ms :f32= 40.0) -> ^fstudio.EVENTINSTANCE {
	using fstudio
...
update_sound_emitters :: proc() {
	// yeet stale guys
	#reverse for &emitter, i in sound_state.sound_emitters {
		if emitter.last_update_tick != sound_state.sound_ticks {
		
			ok := sound_stop(emitter.event)
			if !ok {
			...
			ordered_remove(&sound_state.sound_emitters, i)
...
```
*/
sound_play_continuously :: proc(name: string, unique_id: string, pos := INVALID_POS) {
	...
	#reverse for &emitter, i in sound_state.sound_emitters {
		if emitter.unique_id == unique_id {
			...
			emitter.last_update_tick = sound_state.sound_ticks
			...
			if pos != INVALID_POS {
				...
				succ := sound_update_pos(emitter.event, pos)
				if !succ {
					delete_emitter(&emitter)
					ordered_remove(&sound_state.sound_emitters, i)
				}
	...
	emitter.event = sound_play(name, pos=pos)
	emitter.last_update_tick = sound_state.sound_ticks
	emitter.unique_id = strings.clone(unique_id)
	append(&sound_state.sound_emitters, emitter)
...
sound_update_pos :: proc(event: ^fstudio.EVENTINSTANCE, pos: Vec2) -> bool {
	using state, fstudio
